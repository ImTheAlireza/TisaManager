...
TEXT = {
    ...
    "settings": "<b>⚙️ تنظیمات و کانال‌ها</b>\n\nOwner یا Sudo از تنظیمات می‌تواند کانال Telegram یا Bale اضافه/حذف کند. ربات باید دسترسی ارسال داشته باشد.\n\n«تنظیم تأیید پست‌ها» مشخص می‌کند نویسندگان مستقیم منتشر کنند یا ابتدا منتظر تأیید بمانند.\n\nبرای گروه کانال:\n/group نام_گروه شناسه۱,شناسه۲\n\nبرای بررسی سلامت ربات‌ها و کانال‌ها /health را بفرستید یا از «🧰 ابزارها» گزینهٔ «🩺 سلامت ربات‌ها و کانال‌ها» را بزنید؛ در صورت نیاز «🔄 بررسی مجدد» همهٔ ربات‌ها و کانال‌ها را دوباره آزمایش می‌کند.",
    "tools": "<b>📊 تاریخچه و ابزارها</b>\n\n📋 تاریخچه: مشاهده، ویرایش، حذف، کپی و ارسال مجدد.\n📊 گزارش آماری: /stats\n📋 گروه‌های کانال: /groups\n\nوضعیت‌های مهم: کامل، ناقص، ناموفق، پیش‌نویس و در انتظار تأیید.\n\nاگر ارسال به کانالی ناموفق باشد، ربات هر ۱۰ دقیقه به‌صورت خودکار دوباره تلاش می‌کند تا موفق شود.\nاگر ربات بله دوم (BALE_TOKEN_2) پیکربندی شده باشد، تلاش‌ها یکی‌درمیان بین دو ربات بله تقسیم می‌شوند: تلاش اول با ربات ۱، تلاش دوم با ربات ۲، تلاش سوم با ربات ۱ و به همین ترتیب؛ هر دو ربات باید عضو کانال‌ها باشند.\n\nدر صفحهٔ هر پست، تا وقتی تلاش خودکار در صف است دو دکمه دیده می‌شود (نویسنده برای پست‌های خودش، مالک و ادمین اصلی برای همهٔ پست‌ها):\n• ⚡ ارسال فوری مقصدهای ناموفق: همان لحظه فقط مقصدهای ناموفق را دوباره می‌فرستد و در صورت موفقیت پست «کامل» می‌شود.\n• 🚫 توقف تلاش‌های خودکار: تلاش‌های بعدی را لغو می‌کند و وضعیت فعلی به‌عنوان وضعیت نهایی (ناقص) ثبت می‌شود.\n\nاین دو دکمه با «🔁 ارسال مجدد» فرق دارند؛ ارسال مجدد یک کپی جدید از پست می‌سازد ولی این دکمه‌ها همان پست را کامل می‌کنند.",
}
